...
SELECT
  ...
  ?wikipedia
WHERE {
  VALUES (?uri ?name ?safeName ?description ?subnationalGovernments) {
...
GROUP BY ?uri ?name ?safeName ?description ?subnationalGovernments ?headOfGovLabel ?headOfStateLabel ?geoshape ?wikipedia
ORDER BY ?name
```
